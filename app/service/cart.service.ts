...
this.getOrdersByUser();
   ...
   private baseUrl="http://localhost:8083/api/order/"
  ...
  getTotalAmounOfTheCart(){
    ...
    for(var o in obj ){
      totalPrice = totalPrice +parseFloat(obj[o].prix_de_vente);
    }
...
getOrder(){
  this.http.postRequestWithToken("api/order/getorder",{}).subscribe((data:any)=>{
   //alert("Error while fetching the cart Details");
   this.commandes = data;
   this.CmdServiceEvent.next({"status":"completed"})//emitter
  })
